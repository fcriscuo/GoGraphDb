...
object GoRelationshipDao {
    ...
    private fun loadGoTermRelationship(goId: String, goRel: Relationship) {
        val cypher = relationshipCypher.replace("SOURCE", Neo4jUtils.formatPropertyValue(goId))
            .replace("TARGET", Neo4jUtils.formatPropertyValue(goRel.targetId))
            .replace("RELATIONSHIP", goRel.type.uppercase())
            .replace("DESCRIPTION", Neo4jUtils.formatPropertyValue(goRel.description))
